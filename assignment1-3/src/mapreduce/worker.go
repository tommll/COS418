...
func (wk *Worker) ReceiveHeartBeat(sequence string, response *string) error {
	t := time.Now()
	arg := strings.Split(wk.name, "-")
	*response = arg[2] + " received " + sequence + "th heart beat at micro-second " + strconv.Itoa(int(t.UnixMicro()))
	return nil
}
...
func RunWorker(MasterAddress string, me string,
	...
	for {
		...
		if err == nil {
			// --- added for debugging ---
			//fmt.Printf("Worker %s has %d turns left\n", strings.Split(me, "-")[2], wk.nRPC-1)
			wk.Lock()
			wk.nRPC--
			wk.Unlock()
