...
func (sim *Simulator) StartSnapshot(serverId string) {
	...
	sim.nextSnapshotId++
	sim.logger.RecordEvent(sim.servers[serverId], StartSnapshot{serverId, snapshotId}) // record its own states
	// TODO: IMPLEMENT ME
...
func (sim *Simulator) CollectSnapshot(snapshotId int) *SnapshotState {
	// TODO: IMPLEMENT ME
	serverToken := make(map[string]int)
	snaps := make([]*SnapshotMessage, 0)

	for key, server := range sim.servers{
		serverToken[key] = server.Tokens
	}

	for x := range sim.log.event{
		snaps[]
	}

	snap := SnapshotState{snapshotId, serverToken, make([]*SnapshotMessage, 0)}
	return &snap
}
