...
func TestReElection(t *testing.T) {
	...
	log.Println("Test: election after network failure...")

	leader1 := cfg.checkOneLeader()

	// if the leader disconnects, a new one should be elected.
	log.Println("LEADER DISCONNECTED", leader1)
	...
	log.Println("LEADER RECONNECTED", leader1)
	cfg.connect(leader1)
	...
	// be elected.
	log.Println("LEADER DISCONNECTED", leader2)
	cfg.disconnect(leader2)

	randServer := (leader2 + 1) % servers
	log.Println("SERVER DISCONNECTED", randServer)
	...
	randServer = (leader2 + 1) % servers
	log.Println("SERVER CONNECTED", randServer)
	cfg.connect(randServer)
	cfg.checkOneLeader()
	log.Println("FLAG 4 PASSED")

	// re-join of last node shouldn't prevent leader from existing.
	log.Println("LEADER CONNECTED", leader2)
	cfg.connect(leader2)
	cfg.checkOneLeader()
...
func TestFailAgree(t *testing.T) {
	...
	randServer = (leader + 1) % servers
	log.Println("<<<<<<LEADER CONNECTED", randServer)
	cfg.connect(randServer)
