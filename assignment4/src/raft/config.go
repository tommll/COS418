...
func (cfg *config) start1(i int) {
	...
	fmt.Printf("Create Raft server %d\n", i)
...
func (cfg *config) checkOneLeader() int {
	fmt.Println("Checking one leader")
	...
	for iters := 0; iters < 10; iters++ {
		...
		if len(leaders) != 0 {
			fmt.Println("Finished checking one leader")
...
func (cfg *config) nCommitted(index int) (int, interface{}) {
	log.Printf("Check cmd at index %d\n", index)
	...
	for i := 0; i < len(cfg.rafts); i++ {
		...
		log.Printf("====CHECK CMD at %d of server %d %v\n", index, i, cfg.logs[i])
		cfg.mu.Unlock()

		if ok {
			if count > 0 && cmd != cmd1 {
				cfg.t.Fatalf("committed values do not match: count: %d index %v, %v, %v\n",
					count, index, cmd, cmd1)
			} else {
				log.Printf("committed values match: count: %d index %v, %v, %v\n", count, index, cmd, cmd1)
			}
			count += 1
			cmd = cmd1
		}
	...
	for time.Since(t0).Seconds() < 10 {
		...
		if index != -1 {
			...
			for time.Since(t1).Seconds() < 2 {
				log.Printf(">>>Check index %d\n", index)
				nd, cmd1 := cfg.nCommitted(index)
				log.Printf(">>>Check index %d, cmd: %d result: (%d-%d)\n", index, cmd, nd, cmd1)
